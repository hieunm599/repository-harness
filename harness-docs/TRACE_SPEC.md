...
| `files_read` | TEXT | Tiêu chuẩn trở lên | Văn bản dạng mảng JSON chứa các đường dẫn hoặc tên lệnh. Với CLI hiện tại, truyền một danh sách phân tách bằng dấu phẩy. | `["PHASE2.md","harness-docs/HARNESS.md","scripts/bin/harness-cli query matrix"]` |
| `files_changed` | TEXT | Tiêu chuẩn trở lên | Văn bản dạng mảng JSON chứa các đường dẫn file thay đổi. Với CLI hiện tại, truyền một danh sách phân tách bằng dấu phẩy; chỉ bỏ qua khi không có file nào thay đổi. | `["harness-docs/TRACE_SPEC.md","harness-docs/HARNESS.md"]` |
...
Đối với công việc rủi ro cao, trường `decisions_made` trong trace tóm tắt những gì đã được quyết định. Nó không thay thế cho bản ghi quyết định kỹ thuật lâu dài. Nếu công việc thay đổi hành vi, kiến trúc, phân quyền, quyền sở hữu dữ liệu, cấu trúc API hoặc các yêu cầu xác thực, hãy thêm một file `harness-docs/decisions/NNNN-*.md` và ghi lại nó bằng lệnh `scripts/bin/harness-cli decision add`.
...
scripts/bin/harness-cli trace \
  ...
  --read "harness-docs/product/permissions.md,harness-docs/decisions/0008-auth-boundary.md,src/auth/roles.ts" \
...
scripts/bin/harness-cli trace \
  ...
  --read "PHASE2.md,harness-docs/HARNESS.md,scripts/schema/001-init.sql" \
  --changed "harness-docs/TRACE_SPEC.md,harness-docs/HARNESS.md" \
  --friction "none"
```
