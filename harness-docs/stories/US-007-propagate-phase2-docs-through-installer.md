...
references `harness-docs/CONTEXT_RULES.md`.
...
- The installer payload includes:
  ...
  - `harness-docs/HARNESS_MATURITY.md`
  - `harness-docs/TRACE_SPEC.md`
- The hardcoded AGENTS shim refresh block includes `harness-docs/CONTEXT_RULES.md`.
...
- A merge install with `--refresh-agent-shim` refreshes the marked Harness
  block to include `harness-docs/CONTEXT_RULES.md`.
...
- `rg -n "harness-docs/(CONTEXT_RULES|HARNESS_COMPONENTS|HARNESS_MATURITY|TRACE_SPEC)\.md" scripts/install-harness.sh`
- `rg -n "harness-docs/CONTEXT_RULES.md" scripts/install-harness.sh AGENTS.md`
- Fresh local install into a temporary target using a local
  ...
  with `harness-docs/CONTEXT_RULES.md`, and installed an executable
  `scripts/bin/harness-cli`.
- Merge install into a temporary existing Harness target created the missing
  Phase 2 docs while preserving existing `harness-docs/README.md` and
  `scripts/bin/harness-cli`.
